  ...
  exceedsBudget(expenseToSave: { amount: number; category: string; date: string | Date }): boolean {
    ...
    if (budget) {
      ...
      if (categoryBudget) {
        ...
        if (expenseToSave.amount > categoryBudget.limit) {
          console.log('Expense exceeds category budget limit');
          this.warningMessage = `You are about to add an expense of $${expenseToSave.amount} for ${expenseToSave.category}, but your budget for this category is $${categoryBudget.limit}. Do you want to proceed?`;
